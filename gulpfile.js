...
var less = require('gulp-less');//下载完成以后,就会在node_module中查看到有gulp_less文件 然后在这里引用
...
gulp.task('html',function () {
    /*要操作哪些文件 确定源文件地址*/
    gulp.src(app.srcPath+'**/*.html')  /*src下所有目录下的所有.html文件  **所有文件夹 *所有文件 */  //对他进行操作,通过管道付进行操作
        .pipe(gulp.dest(app.buildPath)) //gulp.dest 要把文件放到指定的目标位置
        .pipe(gulp.dest(app.distPath))
        .pipe(connect.reload()) //当内容发生改变时， 重新加载。  我ctrl+s的时候就可以在浏览器中看到效果
});
...
gulp.task('less',function () {
    gulp.src(app.srcPath+'style/index.less')
        .pipe(less())
        //less是上面的require中的less
        .pipe(gulp.dest(app.buildPath+'css/')) //放的位置
        /*经过压缩，放到dist目录当中*/
        .pipe(cssmin()) //压缩
...
gulp.task('js',function () {
   gulp.src(app.srcPath+'js/**/*.js')
       .pipe(concat('index.js'))//合并所有的js文件 把所有的js文件压缩成一个文件当中,自动生成index.js 然后放到build目录当中
